...
export = (injectedStore: typeof StoreType) => {
  ...
  const list = async (
        ...
        arrayStr.map((subItem) => {
          filter = {
            ...
            items: provider
              ...
              : [
                  {
                    column: Columns.prodPrincipal.name,
                    object: String(subItem),
                  },
                  {
    ...
    if (conID) {
      ...
      if (page) {
        ...
        const data: Array<IProdPrinc> = await store.list(
          Tables.PRODUCTS_PRINCIPAL,
          [ESelectFunct.all],
          filters,
          undefined,
  ...
  const upsert = async (body: INewProduct, listImgDelete?: Array<string>) => {
    if (body.id_prod) {
      ...
      await upsertVariedades(
        JSON.parse(String(body.variedades)),
        true,
        body.id_prod,
      );
        ...
        if (listImgDelete) {
          try {
            listImgDelete.map(async (img) => {
              await store.remove2(
                Tables.PRODUCTS_IMG,
                `url_img= '${img}' AND id_prod= '${body.id_prod}'`,
              );
            });
          } catch (error) {
            await store.remove2(
              Tables.PRODUCTS_IMG,
              `url_img= '${listImgDelete}' AND id_prod= ${body.id_prod}`,
            );
          }
        }
      ...
      const result: INewInsert = await store.insert(
        Tables.PRODUCTS_PRINCIPAL,
        product,
      );
      await upsertVariedades(
  ...
  const remove = async (id_prod: number) => {
    await store
      .remove(Tables.PRODUCTS_PRINCIPAL, { id_prod: id_prod })
  ...
  const get = async (id: number) => {
    const productGral = await store.query(Tables.PRODUCTS_PRINCIPAL, {
      id_prod: id,
    });
    ...
    const productPrices = await store.query(Tables.PRODUCTS_VAR, {
      id_prod: id,
    });
  ...
  const getTags = async () => {
    ...
    return await store.list(
      Tables.PRODUCTS_TAGS,
      [Columns.prodTags.tag],
      undefined,
      groupBy,
      undefined,
      undefined,
  ...
  const varCost = async (
    ...
    const updateCol: Array<IWhere> = [
      {
        ...
        object: String(
          `ROUND((${Columns.prodPrincipal.costo} + (${Columns.prodPrincipal.costo})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
      {
        column: Columns.prodPrincipal.minorista,
        object: String(
          `ROUND((${Columns.prodPrincipal.minorista} + (${Columns.prodPrincipal.minorista})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
      {
        column: Columns.prodPrincipal.mayorista_1,
        object: String(
          `ROUND((${Columns.prodPrincipal.mayorista_1} + (${Columns.prodPrincipal.mayorista_1})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
      {
        column: Columns.prodPrincipal.mayorista_2,
        object: String(
          `ROUND((${Columns.prodPrincipal.mayorista_2} + (${Columns.prodPrincipal.mayorista_2})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
      {
        column: Columns.prodPrincipal.mayorista_3,
        object: String(
          `ROUND((${Columns.prodPrincipal.mayorista_3} + (${Columns.prodPrincipal.mayorista_3})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
      {
        column: Columns.prodPrincipal.revendedor,
        object: String(
          `ROUND((${Columns.prodPrincipal.revendedor} + (${Columns.prodPrincipal.revendedor})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
      {
        column: Columns.prodPrincipal.supermercado,
        object: String(
          `ROUND((${Columns.prodPrincipal.supermercado} + (${Columns.prodPrincipal.supermercado})* ${aumentoFinal}), ${roundNumber})`,
        ),
      },
    ];
    let filter: IWhereParams | undefined = undefined;
    ...
    return await store.updateWhere(
      Tables.PRODUCTS_PRINCIPAL,
      updateCol,
      filters,
    );
  ...
  const updateCost = async (idProd: number, cost: number, oldCost: number) => {
    ...
    const updateCol: Array<IWhere> = [
      ...
      },
      {
        column: Columns.prodPrincipal.minorista,
        object: String(
          `ROUND((${Columns.prodPrincipal.minorista} + (${Columns.prodPrincipal.minorista})* ${percentage}), 2)`,
        ),
      },
      {
        column: Columns.prodPrincipal.mayorista_1,
        object: String(
          `ROUND((${Columns.prodPrincipal.mayorista_1} + (${Columns.prodPrincipal.mayorista_1})* ${percentage}), 2)`,
        ),
      },
      {
        column: Columns.prodPrincipal.mayorista_2,
        object: String(
          `ROUND((${Columns.prodPrincipal.mayorista_2} + (${Columns.prodPrincipal.mayorista_2})* ${percentage}), 2)`,
        ),
      },
      {
        column: Columns.prodPrincipal.mayorista_3,
        object: String(
          `ROUND((${Columns.prodPrincipal.mayorista_3} + (${Columns.prodPrincipal.mayorista_3})* ${percentage}), 2)`,
        ),
      },
      {
        column: Columns.prodPrincipal.revendedor,
        object: String(
          `ROUND((${Columns.prodPrincipal.revendedor} + (${Columns.prodPrincipal.revendedor})* ${percentage}), 2)`,
        ),
      },
      {
        column: Columns.prodPrincipal.supermercado,
        object: String(
          `ROUND((${Columns.prodPrincipal.supermercado} + (${Columns.prodPrincipal.supermercado})* ${percentage}), 2)`,
        ),
      },
    ];
    ...
    return new Promise((resolve, reject) => {
      ...
      lista.map(async (item, key) => {
        ...
        const variation: any = await new Promise(async (resolve, reject) => {
          let varList: Array<IProdVar> = await store.get(
            Tables.PRODUCTS_VAR,
            item.id_prod || 0,
            'id_prod',
          );
  ...
  const getImagesProduct = async (prodId: number) => {
    ...
    filter = {
      mode: EModeWhere.strict,
      concat: EConcatWhere.none,
      items: [{ column: Columns.prodImg.id_prod, object: String(prodId) }],
    };
  ...
  const correctImages = async () => {
    const dataProduct: Array<IProdPrinc> = await store.list(
      Tables.PRODUCTS_PRINCIPAL,
      ['*'],
    );
    ...
    dataProduct.map(async (item, key) => {
      ...
      const imageList: Array<IImgProd> = await store.query(
        Tables.PRODUCTS_IMG,
        { id_prod: idProd },
      );
  ...
  const correctVariedades = async () => {
    ...
    return new Promise((resolve, reject) => {
      dataProduct.map(async (item, key) => {
        const variedades: Array<IProdVar> = await store.query(
          Tables.PRODUCTS_VAR,
