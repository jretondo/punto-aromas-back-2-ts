...
export = (injectedStore: typeof StoreType) => {
  ...
  const sellerList = async () => {
    ...
    const filter: IWhereParams | undefined = {
      ...
      items: [{ column: Columns.admin.pv, object: String('') }],
    };
    filters.push(filter);
    const data = await store.list(
      Tables.ADMIN,
  ...
  const listCtaCteSeller = async (
    ...
    if (pendiente) {
      filter = {
        ...
        items: [
          {
            column: Columns.facturas.id_seller_comision,
            object: String(idVendedor),
          },
      ...
      filter = {
        ...
        items: [
          ...
          {
            column: `${Columns.facturas.comision} - ${Columns.facturas.comision_paga}`,
            object: String(0),
          },
        ],
      };
      ...
      filter = {
        ...
        concat: EConcatWhere.and,
        items: [{ column: Columns.facturas.comision, object: String(0) }],
      };
      filters.push(filter);
    } else {
      filter = {
    ...
    if (page) {
      ...
      const suma = await store.list(
        Tables.FACTURAS,
        [
          `SUM(${Columns.facturas.comision} - ${Columns.facturas.comision_paga}) as SUMA`,
        ],
        filters,
      );
  ...
  const getDetailsFact = async (idFact: number) => {
    ...
    const suma = await store.list(
      Tables.FACTURAS,
      [`SUM(${Columns.facturas.comision}) as SUMA`],
      filters,
    );
  ...
  const clientsList = async (userId: number, page: number) => {
    ...
    filter = {
      mode: EModeWhere.strict,
      concat: EConcatWhere.and,
      items: [{ column: Columns.clientes.vendedor_id, object: String(userId) }],
    };
    filters.push(filter);
    const data = await store.list(
      Tables.CLIENTES,
  ...
  const deleteClient = async (clientId: number) => {
    const result: INewInsert = await store.update(
      Tables.CLIENTES,
      { vendedor_id: null },
      clientId,
    );
