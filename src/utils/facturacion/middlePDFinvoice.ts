    ...
    const middleware = async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (newFact.fiscal) {
          ...
          if ([CbteTipos['Nota de Crédito A'], CbteTipos['Nota de Crédito B'], CbteTipos['Nota de Crédito C'], CbteTipos['Nota de Crédito M']].includes(dataFiscal.CbteTipo)) {
            const cbteAsocObj = dataFiscal.CbtesAsoc?.CbtesAsoc?.[0] || { PtoVta: 0, Nro: 0 };
            cbteAsoc = `${zfill(cbteAsocObj.PtoVta, 5)} - ${zfill(cbteAsocObj.Nro, 8)}`;
          }
        ...
        const totales = {
          subTotal: formatMoney(Math.abs(newFact.total_neto - newFact.costo_envio) + newFact.descuento),
          subTotalNoFiscal: formatMoney(Math.abs(newFact.total_neto - newFact.costo_envio + newFact.total_iva) + newFact.descuento),
        ...
        const ejsPath = path.join('views', 'invoices', newFact.fiscal ? 'Factura.ejs' : 'FacturaNoFiscal.ejs');
