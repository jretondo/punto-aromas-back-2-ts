...
const factuMiddel = () => {
  const middleware = async (
    ...
    try {
      ...
      const clienteData: Array<IClientes> = await clientesController.getCuit2(
        body.cliente_ndoc || 0,
      );
      ...
      if (descuento !== 0) {
        descuentoNumber =
          Math.round(
            (productsList.totalFact - body.costoEnvio) *
              (descuento / 100) *
              100,
          ) / 100;
        ...
        productsList.totalFact =
          productsList.totalFact -
          (productsList.totalFact - body.costoEnvio) * (descuento / 100);
        productsList.totalIva =
          productsList.totalIva -
          (productsList.totalIva - costoIvaEnvio) * (descuento / 100);
        productsList.totalNeto =
          productsList.totalNeto -
          (productsList.totalNeto - netoEnvio) * (descuento / 100);
      }
      ...
      comision =
        (productsList.totalFact - productsList.totalReventa) * porcPago;
      comisionImputar =
        productsList.totalFact - productsList.totalReventa - comision;
  ...
  return new Promise((resolve, reject) => {
    ...
    productsList.map(async (prod, key) => {
      ...
      const totalCosto: number =
        Math.round(dataProd[0].costo * prod.cant_prod * 100) / 100;
      const totalProd: number =
        Math.round(prod.price * prod.cant_prod * 100) / 100;
      const totalNeto: number =
        Math.round((totalProd / (1 + dataProd[0].iva / 100)) * 100) / 100;
      const totalIva: number =
        Math.round(totalNeto * (dataProd[0].iva / 100) * 100) / 100;
      const totalRevende: number =
        Math.round(dataProd[0].revendedor * prod.cant_prod * 100) / 100;
      ...
      factura.totalNeto =
        Math.round((factura.totalNeto + totalNeto) * 100) / 100;
      factura.totalCosto =
        Math.round((factura.totalCosto + totalCosto) * 100) / 100;
      factura.totalReventa =
        Math.round((factura.totalReventa + totalRevende) * 100) / 100;
      ...
      if (key === productsList.length - 1) {
        ...
        factura.totalFact =
          Math.round((factura.totalFact + Number(costoEnvio)) * 100) / 100;
        factura.totalIva =
          Math.round((factura.totalIva + Number(costoIvaEnvio)) * 100) / 100;
        factura.totalNeto =
          Math.round((factura.totalNeto + Number(netoEnvio)) * 100) / 100;
        resolve(factura);
      }
    });
  });
        ...
        if (iva !== ivaAnt) {
          if (descuento > 0) {
            listaIva.push({
              ...
              BaseImp:
                Math.round(
                  (item.total_neto - item.total_neto * (descuento / 100)) * 100,
                ) / 100,
              Importe:
                Math.round(
                  (item.total_iva - item.total_iva * (descuento / 100)) * 100,
                ) / 100,
            });
          } else {
            listaIva.push({
          ...
          if (descuento > 0) {
            ...
            listaIva[index] = {
              Id: iva,
              BaseImp:
                Math.round((listaIva[index].BaseImp + item.total_neto) * 100) /
                100,
              Importe:
                Math.round((listaIva[index].Importe + item.total_iva) * 100) /
                100,
            };
          }
        }
